...
def test_hole_correlations_against_precalculated_paulina():
    ...
    diffs = _get_correlation_diffs(folder / "hole_correlations_paulina.txt")
...
def test_hole_correlations_against_precalculated_scco():
    ...
    folder = pathlib.Path(__file__).parent / "precalculated"
    diffs = _get_correlation_diffs(folder / "hole_correlations_scco.txt")
    listed_precision = 1e-3
...
def _get_correlation_diffs(path: pathlib.Path) -> list[np.ndarray]:
...
def test_singlet_singlet_correlations_against_precalculated():
    """Calculated 2- and 3-hole correlations should match values published in my dissertation."""
    pass
